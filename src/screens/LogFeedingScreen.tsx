...
interface LogFeedingScreenProps {
  navigation: any;
  route: any;
}
...
export const LogFeedingScreen: React.FC<LogFeedingScreenProps> = ({ navigation, route }) => {
  const { starterId } = route.params;
  const { getStarterById, addFeeding } = useStarterStore();
  
  const [starter] = useState(() => getStarterById(starterId));
  ...
  useEffect(() => {
    // Calculate initial weights based on default ratio
    calculateWeights(selectedRatio);
  }, []);
